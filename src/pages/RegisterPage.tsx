...
import { BASE_URL, apiRegister, authStorage } from '../lib/api';
...
export default function RegisterPage() {
  ...
  useEffect(() => {
    // Precalentamos el backend para que el registro sea inmediato al enviar el formulario
      ...
      <div className="w-full max-w-md rounded-xl bg-slate-900/95 p-8 shadow-2xl ring-1 ring-black/10">
        ...
        <form
          ...
          onSubmit={async (e) => {
            ...
            setLoading(true);
            try {
              const res = await apiRegister({ name, identifier, password });
        ...
        >
          ...
          <div>
            <label className="mb-2 block text-sm font-medium text-slate-200">Correo Electrónico o Número de Teléfono</label>
            <input
              type="text"
              value={identifier}
        ...
        <p className="mt-6 text-center text-sm text-slate-400">
          ¿Ya tienes cuenta?{' '}
          <Link to="/app/login" className="font-medium text-slate-200 hover:underline">
            Inicia sesión
          </Link>
        </p>
      </div>
